...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const matchingTodo = await findTodoByUuid({ db, uuid: args.uuid });
    if (matchingTodo) {
      const currentList = await db.get(matchingTodo.list_id);
      if (!currentList) {
        throw mutationError(MUTATION_ERROR_CODES.NOT_FOUND, `No matching list found for todo uuid=${args.uuid}`);
      }
      assertListOwner(currentList, ownerId);
      await db.patch(matchingTodo._id, {
        ...args,
        list_id: matchingList._id
      });
      return matchingTodo._id;
    }
